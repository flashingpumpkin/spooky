...
%% Created: 26 Jun 2011
%% Description: Tests for PUT method
-module(spooky_put_test).
...
root()->
    ?assertEqual(200, status(request('PUT'))).

path()->
    ?assertEqual(200, status(request('PUT',"/androids"))).

invalid_route()->
    ?assertEqual(404, status(request('PUT',"/androids/sheep"))).
